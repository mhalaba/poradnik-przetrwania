...
const labels=[];
function makeLabel(text,x,y,z,size=1,bg='rgba(10,16,30,.62)',maxDist=55){ const cv=document.createElement('canvas'); cv.width=512; cv.height=112; const cx=cv.getContext('2d'); cx.fillStyle=bg; if(cx.roundRect){cx.beginPath(); cx.roundRect(2,2,508,108,38); cx.fill();} else cx.fillRect(0,0,512,112); cx.strokeStyle='rgba(255,255,255,.25)'; cx.lineWidth=3; if(cx.roundRect){cx.stroke();} cx.fillStyle='#fff'; cx.font='bold 46px Nunito, Inter, sans-serif'; cx.textAlign='center'; cx.textBaseline='middle'; cx.fillText(text,256,58); const t=new THREE.CanvasTexture(cv); const sp=new THREE.Sprite(new THREE.SpriteMaterial({map:t,transparent:true,depthTest:false})); sp.position.set(x,y,z); sp.scale.set(6.4*size,1.4*size,1); sp.userData={maxDist,base:1}; world.add(sp); labels.push(sp); return sp; }
...
function drawMinimap(){ if(!mmc) return; const S=mm.width, C=S/2, px=player.position.x, pz=player.position.z, k=C/MMR;
  ...
  MISSIONS.forEach(m=>{ const done=state.done[m.id], next=!done&&MISSIONS.filter(x=>x.id<m.id).every(x=>state.done[x.id]);
    mmc.fillStyle=done?'#2fd27a':next?(KIDS?'#ff6b9d':'#f5b400'):'rgba(160,175,195,.75)';
    mmc.beginPath(); mmc.arc(X(m.pos[0]),Z(m.pos[1]),next?5:3.2,0,6.2832); mmc.fill(); });
...
function finishQuest(st){ const m=quest.mission,ch=DATA[m.id]; let stars=quest.stars; if(quest.total>0){ const r=quest.ok/quest.total; if(r<.5) stars=Math.min(stars,1); else if(r<.85) stars=Math.min(stars,2); } const first=!state.done[m.id]; const score=60+stars*40+Math.round(state.calm/5);
  ...
  open('<h2>'+(KIDS?'🎉 Misja ukończona!':'✅ Misja zaliczona')+'</h2><div class="stars">'+'⭐'.repeat(stars)+'☆'.repeat(3-stars)+'</div><p>'+(st.summary||'')+'</p><p><b>+'+gained+' pkt</b>'+(KIDS?' · odznaka '+ch.odznaka:'')+'</p>'+(st.plan?'<div class="tip"><b>'+T('🧠 Twój plan „jeśli–to”:','🧠 Zapamiętaj:')+'</b> '+st.plan+'<br><label style="display:flex;gap:8px;align-items:center;margin-top:8px;cursor:pointer"><input type="checkbox" id="commit" style="width:20px;height:20px"> '+T('Zobowiązuję się – wdrożę to w tym tygodniu (+20 pkt)','Obiecuję, że to zapamiętam! (+20)')+'</label></div>':'')+'<div class="msg"><b>📖 Z książki – '+ch.nr+': '+ch.tytul+'</b><ul>'+learn.map(p=>'<li>'+p+'</li>').join('')+'</ul><a class="btn sm ghost" href="szkolenie.html?wersja='+(KIDS?'dzieci':'dorosli')+'&r='+m.id+'" target="_blank">Cały rozdział w szkoleniu</a></div><div class="navbtns" style="justify-content:center"><button class="btn primary" id="okBtn">Wracam do miasteczka →</button></div>'+(Object.keys(state.done).length===MISSIONS.length?'<p style="text-align:center"><button class="btn ghost" id="endBtn">🏁 Podsumowanie gry</button></p>':''));
  $('#okBtn').onclick=()=>{ const c=$('#commit'); if(c&&c.checked){ state.score+=20; save(); refreshBeacons(); TR('zobowiazanie',{misja_id:m.id}); toast('💪 +20 pkt za zobowiązanie. Wpisz to do kalendarza!'); } close(); }; const e=$('#endBtn'); if(e) e.onclick=showEnd; }
...
function buildSteps(m){ const ch=DATA[m.id]; const D=[];
 ...
 case 1: D.push({type:'talk',npc:'dziadek',...at(0,8),auto:true,lines:[{npc:'dziadek',text:T('Zanim kupisz latarkę – zadaj sobie pytanie: czego naprawdę się obawiam? Dom stoi 300 m od rzeki, ja mam 79 lat i leki na serce, Zosia ma 8 lat. Obejdź dom i zaznacz słabe punkty.','Detektyw zagrożeń najpierw patrzy i myśli. Obejdź dom i znajdź 4 miejsca, na które trzeba uważać.')}]});
  ...
  D.push({type:'panel',run:()=>KIDS?quizPanel(ch.quiz,null,()=>nextStep()):orderPanel('Matryca: priorytet = prawdopodobieństwo × skutki. Ułóż od najpilniejszego.',['Powódź (P=4, S=5) = 20','Cyberatak na bank (P=5, S=3) = 15','Blackout (P=3, S=4) = 12','Zamieszki (P=2, S=5) = 10'],'Wynik powyżej 12 = obszar krytyczny.',()=>nextStep())});
 ...
 case 2: D.push({type:'talk',npc:'ania',x:-20,z:14,auto:true,lines:[{npc:'ania',text:T('Alarm chemiczny w fabryce. Ty w pracy, ja na zakupach, Zosia w szkole. Gdzie się spotkamy? Ustalmy to TERAZ, a nie w panice.','Jak zgubimy się w kryzysie, gdzie się spotkamy? Ustalmy to razem!'),choice:[{t:T('Dwa miejsca: A – plac sąsiedzki, B – dom cioci w Gliwicach.','Dwa miejsca: plac z ławkami i dom babci.'),ok:true,why:'Miejsce A blisko domu (pożar), B poza okolicą (ewakuacja).'},{t:T('Po prostu do siebie zadzwonimy.','Zadzwonię do mamy.'),ok:false,why:'Sieci GSM się przeciążają. Plan łączności PACE zakłada brak technologii.'}]},
   {npc:'zosia',text:'A jak ktoś obcy powie, że mama go przysłała po mnie?',choice:[{t:T('Ustalamy HASŁO BEZPIECZEŃSTWA. Bez hasła nie idziesz z nikim.','Mamy tajne hasło rodzinne! Bez hasła nie idziesz z nikim.'),ok:true,why:'Safe word z szablonu Family Emergency Plan.'},{t:T('Zosia jest mądra, pozna po człowieku.','Idziesz, jeśli jest miły.'),ok:false,why:'Dzieci są celem manipulacji. Hasło to prosta, sprawdzalna reguła.'}]},
   {npc:'dziadek',text:'Kto co robi, gdy padnie komenda? Przydziel role.',choice:[{t:T('Ania – apteczka i leki, ja – dokumenty i media, Zosia – Burek i „nienudne pudełko”, dziadek – radio i kontakt z ciocią.','Mama – apteczka, tata – dokumenty, ja – Burek i pudełko na nudę, dziadek – radio.'),ok:true,why:T('Każdy ma rolę – nikt nie jest bezradny. Dziecko z zadaniem panikuje mniej.','Dziecko z zadaniem czuje się silne!')},{t:T('Ja zrobię wszystko, reszta niech się nie stresuje.','Dorośli zrobią wszystko.'),ok:false,why:'Bezradność potęguje panikę. Nadaj rolę nawet 8-latce.'}]}]});
 ...
 case 6: D.push({type:'talk',npc:'strazak',x:0,z:-69,auto:true,lines:[{npc:'strazak',text:T('Sobota, cisza – i nagle syrena. Większość ludzi zamiera albo dzwoni do wszystkich. Ty masz rozpoznać sygnał w 10 sekund i działać według planu. Panika bywa groźniejsza niż sam kryzys.','Syrena to „dźwięk ochrony”. Nie straszy – ostrzega. Naucz się, co mówi.')}]});
  ...
  D.push({type:'end',summary:T('Modulowany = alarm, ciągły = odwołanie, przerywany = test (pierwszy poniedziałek).','Znasz dźwięki ochrony!'),plan:T('Jeśli słyszę syrenę modulowaną, to włączam radio, sprawdzam RCB i idę do pokoju schronienia.','Jeśli słyszę syrenę, to idę do dorosłego i słucham.')}); break;
...
function showIntro(){ open('<div id="intro"><div style="font-size:3rem">'+(KIDS?'🧒🎮':'🧭🎮')+'</div><h2>'+T('Rodzina Nowaków: 19 misji przetrwania','Miasteczko Małego Strażnika')+'</h2><p>'+T('Jesteś koordynatorem rodziny: Ania, Zosia (8 l.), dziadek Józef (79 l.) i pies Burek. Dom 300 m od rzeki. Przejdziesz przez wszystkie rozdziały książki „'+KSIAZKA.tytul+'”: od analizy ryzyka, przez zapasy, syreny, ewakuację, blackout i powódź, aż po odbudowę. Każde zadanie wykonujesz naprawdę: idziesz, szukasz, decydujesz, ćwiczysz refleks i oddech.','Mieszkasz z mamą Anią, dziadkiem Józefem i psem Burkiem. Zostaniesz Małym Strażnikiem Bezpieczeństwa: pakujesz plecak, słuchasz syren, budujesz bazę, uciekasz przed wodą na górkę i łapiesz fake newsy. Za każdą misję – gwiazdki i odznaka!')+'</p><p class="small"><b>Wskaźnik spokoju:</b> '+T('złe decyzje podnoszą stres, oddech 4-7-8 go obniża. Panika bywa groźniejsza niż kryzys.','gdy się boisz, oddychaj jak na gorącą zupę – wskaźnik rośnie!')+'</p><p class="small"><b>Sterowanie:</b> WASD / strzałki – ruch · przeciągnij myszą lub Q – obrót kamery · <b>kółko myszy, klawisze + / − lub przyciski z boku – przybliżanie</b> (0 = widok domyślny) · E lub przycisk – działanie.<br>Telefon: joystick po lewej, przeciąganie – kamera, <b>szczypanie dwoma palcami – zoom</b>.</p>'+missionList()+'<div class="navbtns" style="justify-content:center"><button class="btn primary" id="st">▶ Graj</button><a class="btn ghost" href="szkolenie.html?wersja='+(KIDS?'dzieci':'dorosli')+'">📚 Najpierw szkolenie</a></div></div>'); $('#st').onclick=()=>{ audio(); TR('gra_start',{ukonczone:Object.keys(state.done).length}); close(); if(!Object.keys(state.done).length) toast(T('Zacznij od misji 0 w szkole – żółty budynek na wschodzie (idź w prawo drogą).','Zacznij od misji 0 – idź do szkoły, żółty budynek po prawej!')); }; }
